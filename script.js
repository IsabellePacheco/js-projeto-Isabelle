...
menuIcon.addEventListener('click',()=> {
    ...
    document.body.style.overflow = navList.classList.contain('open') ? 'hidden' : 'auto';
 });
...
// Seleciona o elemento 'título' e define as variáveis paraanimação
...
const ections = document.querySelectorAll ('section');

sections. forEach((section, index)=> {
    section.style.opacity = '0';
    section.style.transition = 'opacity 1s, transform 1s';

    // aplica diferentes transformações de acordo com o indice de seção
    if (index !==0){
        if (index === 1) section.style.transform = 'traslate(100px)';
